...
const ClientCard: React.FC<{ 
    ...
    const linkedProperties = (client.interestedPropertyIds || []).map(id => {
...
export const CRMPage: React.FC = () => {
  ...
  const runMatchAnalysis = async (client: Client) => {
      if(!client) return;
      setIsMatching(true);
        ...
        <div className="flex items-center gap-4 mb-4 border-b border-slate-200 pb-1 overflow-x-auto">
            {pipelines.map(p => (
                <button
                    ...
                    className={`px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors border-b-2 ${currentPipelineId === p.id ? 'border-primary-600 text-primary-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                    {p.name}
                </button>
            ))}
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden">
            <div className="flex h-full gap-4 min-w-max pb-2">
                ...
                <div 
                    ...
                    onDrop={(e) => { e.preventDefault(); handleDropClient(e.dataTransfer.getData('clientId'), 'lost_zone'); }}
                >
                    <div className="p-4 text-center text-red-400 opacity-60 hover:opacity-100 transition-opacity flex flex-col items-center justify-center h-full gap-2">
                        <Archive size={32} />
                        <span className="font-semibold text-sm">Arrastar para Perda</span>
                    </div>
                </div>
            </div>
        </div>
